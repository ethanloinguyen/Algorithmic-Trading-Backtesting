...
from src.bq_io import read_residuals_for_window, write_pair_results_raw, log_pipeline_run
...
def process_pair(
    ...
    x = residuals_pivot[ticker_i].dropna().values
    y = residuals_pivot[ticker_j].dropna().values
...
def run_pair_job(
    ...
    log_pipeline_run({
        ...
        "cpu_hours_used": 0.0,  # Populated by Cloud Run metrics
        "status": "COMPLETE",
        "error_message": "",
    ...
    logger.info(
        ...
        f"{len(all_results):,} results, {duration:.1f}s"
    )
...
if __name__ == "__main__":
    ...
    # Read environment variables (set by Cloud Run job trigger)
