...
export default function SectorFilter() {
  const [modalStock, setModalStock] = useState<typeof indices[0] | null>(null);
  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-4 mb-6">
        {indices.map((idx) => (
          <div
            ...
            onClick={() => setModalStock(idx)}
            onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.border = "1px solid hsl(215, 20%, 28%)"; (e.currentTarget as HTMLDivElement).style.background = "hsl(215, 25%, 13%)"; }}
            onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.border = "1px solid hsl(215, 20%, 18%)"; (e.currentTarget as HTMLDivElement).style.background = "hsl(215, 25%, 11%)"; }}
          >
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              <svg width="60" height="20" viewBox="0 0 60 20">
                <polyline
                  ...
                  strokeWidth="2"
                  strokeLinecap="round"
                  points={idx.positive ? "0,14 12,12 24,10 36,8 48,9 60,5" : "0,5 12,8 24,9 36,12 48,11 60,14"}
                />
              </svg>
            </div>
      ...
      {modalStock && (
        <StockModal
          stock={{ symbol: modalStock.symbol, name: modalStock.name, price: modalStock.price, change: `${modalStock.change} (${modalStock.pct})`, positive: modalStock.positive }}
