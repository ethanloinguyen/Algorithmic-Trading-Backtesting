...
export default function StockModal({ stock, onClose }: StockModalProps) {
  ...
  // Fetch OHLCV whenever symbol or range changes
  useEffect(() => {
    if (!stock) return;
    ...
    async function fetchOHLCV() {
      ...
      setHoveredIdx(null);
      try {
        const res = await fetch(`${API}/api/stocks/${stock!.symbol}/ohlcv?range=${range}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = await res.json();
        setOhlcv(json.candles);
      } catch (err) {
  ...
  const prices = ohlcv.map(r => parseFloat(r.close)).filter(Boolean);
  const minP = prices.length ? Math.min(...prices) : 0;
  const maxP = prices.length ? Math.max(...prices) : 1;
  const priceRange = maxP - minP || 1;

  const points = prices.map((p, i) => ({
    x: i / Math.max(prices.length - 1, 1),
    y: 1 - (p - minP) / priceRange,
  }));

  const svgPoints = points.map(p =>
    `${PAD + p.x * (W - PAD * 2)},${PAD + p.y * (H - PAD * 2)}`
  ).join(" ");

  const areaPath = prices.length
    ? `M ${PAD},${H - PAD} ` +
      points.map(p => `L ${PAD + p.x * (W - PAD * 2)},${PAD + p.y * (H - PAD * 2)}`).join(" ") +
      ` L ${W - PAD},${H - PAD} Z`
    : "";
    ...
    >
      ...
      >
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span
                ...
                style={{ background: stock.positive ? "hsl(142, 71%, 20%)" : "hsl(0, 84%, 20%)", color }}
              >
                {stock.change}
              </span>
            </div>
        ...
        <div
          ...
          style={{ background: "hsl(215, 25%, 8%)", border: "1px solid hsl(215, 20%, 16%)" }}
        >
          {/* OHLCV hover bar */}
          <div className="px-4 py-2 flex gap-6" style={{ borderBottom: "1px solid hsl(215, 20%, 14%)" }}>
            {hovered
              ...
              : <span className="text-xs" style={{ color: "hsl(215, 15%, 45%)" }}>Loading…</span>
            }
          </div>

          {loading ? (
            <div className="flex items-center justify-center" style={{ height: 200 }}>
              <div className="w-6 h-6 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: "hsl(217, 91%, 60%)", borderTopColor: "transparent" }} />
            </div>
          ) : error ? (
            <div className="flex items-center justify-center text-sm" style={{ height: 200, color: "hsl(0, 84%, 60%)" }}>
              {error}
            </div>
          ) : (
            <svg
            ...
            >
              ...
              {[0.25, 0.5, 0.75].map(f => (
                <line key={f} x1={PAD} y1={PAD + f * (H - PAD * 2)} x2={W - PAD} y2={PAD + f * (H - PAD * 2)}
                  stroke="hsl(215, 20%, 15%)" strokeWidth="1" />
              ))}
              {areaPath && <path d={areaPath} fill="url(#areaGrad)" />}
              {svgPoints && (
                <polyline fill="none" stroke={color} strokeWidth="1.5"
                  strokeLinecap="round" strokeLinejoin="round" points={svgPoints} />
              )}
                ...
                return (
                  <>
                    <line x1={cx} y1={PAD} x2={cx} y2={H - PAD}
                      stroke="hsl(215, 20%, 35%)" strokeWidth="1" strokeDasharray="4,3" />
                    <circle cx={cx} cy={cy} r="4" fill={color}
                      stroke="hsl(215, 28%, 10%)" strokeWidth="2" />
                  </>
                );
              })()}
            </svg>
        ...
        >
          <div
            ...
            style={{ gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1fr 1fr", color: "hsl(215, 15%, 45%)", borderBottom: "1px solid hsl(215, 20%, 14%)" }}
          >
            {["Date", "Open", "High", "Low", "Close", "Volume"].map(h => (
              <span key={h} className={h !== "Date" ? "text-right" : ""}>{h}</span>
            ))}
          </div>
          <div className="overflow-y-auto" style={{ maxHeight: "140px" }}>
            {loading
              ? Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="grid px-4 py-2 animate-pulse"
                  style={{ gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1fr 1fr", borderTop: "1px solid hsl(215, 20%, 13%)" }}>
                  {Array.from({ length: 6 }).map((_, j) => (
                    <div key={j} className="h-3 rounded mx-1" style={{ background: "hsl(215, 20%, 18%)" }} />
                  ))}
                </div>
              ))
              : [...ohlcv].reverse().slice(0, 10).map((row, i) => (
                <div key={i} className="grid px-4 py-1.5 text-xs"
                  style={{ gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1fr 1fr", borderTop: "1px solid hsl(215, 20%, 13%)" }}>
